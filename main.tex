...
\usepackage{lmodern} % Load scalable versions of the Computer Modern fonts
...
For using some of these symbols it's maybe needed to implement the following usepackage;
\begin{table}[h]
    \centering
    \begin{tabular}{|c|c|} \hline
       \rowcolor{gray!30} 
       Usepackage implementation & What the usepackage does \\ \hline
       \bs{usepackage\{amssymb\}} & Used to import symbols for your document \\ \hline 
    \end{tabular}
\end{table}
\subsection{Tables}
There can be many different needs for tables and therefor there also exist many ways of implementing an editing a table in \LaTeX. We will start at the beginning and slowly add more things to enhance the 
visuals of the tables.

\subsection*{The Basic of \LaTeX Tables}
We will start by taking a look at one of the simplest tables you can create in \LaTeX
\begin{multicols}{2}
  \textbf{Setup:}
  \begin{lstlisting}
  \begin{center}
...
We will now look at this example a bit more indepth. The whole center environment wrapped around the tabular environemnt is used to center the table on the page. The tabular environment is the most basic way of creating a table in \LaTeX, when doing so you also must give a parameterr to the environemnt. Here we have given "\{ c c c \}" which states that we want to have three columns, each with the contents centered.
The tabular environemnt also allows for more flexibility. This can be by adding vertical lines to sepperate the columns 
...
Looking at the code from above we can see the following new things.
...
\subsection{Mathematics}
    ...
    \input{TexFiles/Math-Operators.tex}
    \label{secc:Math-Operators}

    \subsubsection{Symbols}
    \input{TexFiles/Math-Symbols.tex}
